...
class LLMClient:
    ...
    def __init__(
        ...
        self.default_timeout = 45  # Reduced from 120s for better UX
    ...
    async def stream_chat_async(
        ...
        Uses proper async/await instead of threading.
        ...
        try:
            if self.provider == LLMProvider.DEEPSEEK and self.deepseek_async_client:
                # DeepSeek has native async streaming
                stream = await self.deepseek_async_client.chat.completions.create(
                    model=self.model,
                    messages=full_messages,
                    temperature=temperature,
                    max_tokens=4096,
                ...
                # Ollama: wrap sync generator in async
                # Using asyncio.to_thread for each chunk avoids blocking
                def get_stream():
