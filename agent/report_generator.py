...
| Online | {online} | {'✅' if online > 0 else '⚠️'} |
| Offline | {offline} | {'❌' if offline > 0 else '✅'} |
| Critical Alerts | {critical} | {'🚨' if critical > 0 else '✅'} |
| Warnings | {warnings} | {'⚠️' if warnings > 0 else '✅'} |
...
"""
        if alerts:
            for a in alerts[:10]:
                icon = "🚨" if a.severity.value == "critical" else "⚠️" if a.severity.value == "warning" else "ℹ️"
                ack = "✓" if a.acknowledged else ""
                report += f"- {icon} **{a.severity.value.upper()}** {ack}: {a.message[:60]}... _{a.timestamp[:16]}_\n"
        else:
            report += "_No recent alerts_ ✅\n"
...
"""
        ...
        if offline > 0:
            report += f"- 🔧 **{offline} device(s) offline** - investigate connectivity issues\n"
        if critical > 0:
            report += f"- 🚨 **{critical} critical alert(s)** - immediate attention required\n"
        if warnings > 0:
            report += f"- ⚠️ **{warnings} warning(s)** - review and address as needed\n"
        if offline == 0 and critical == 0 and warnings == 0:
            report += "- ✅ **All systems healthy** - continue monitoring\n"
...
"""
    ...
    def generate_device_report(self, device_id: str) -> str:
        ...
        report = f"""# 📱 Device Report: {device.name}
...
"""
        if last_check:
            ...
            report += f"**Ping:** {'✅ OK' if last_check.ping_ok else '❌ Failed'}\n"
...
"""
    ...
    def generate_weekly_summary(self) -> str:
        ...
        report = f"""# 📅 Weekly Network Summary
